...
func (u UserServiceImpl) UpdateUserData(c *gin.Context) {
	...
	res, _ := u.userRepository.FindUserById(data.ID)
...
func (u UserServiceImpl) GetUsers(c *gin.Context) {
	...
	if roleIDStr != "" {
		if id, err := strconv.Atoi(roleIDStr); err == nil {
			roleID = &id
			log.Info("Role ID received: ", *roleID)
		} else {
			log.Error("Invalid role_id format: ", roleIDStr)
	...
	data, err := u.userRepository.GetUsersList(roleID)
	if err != nil {
